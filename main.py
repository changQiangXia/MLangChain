...
def print_result(result: dict):
    ...
    print("\n" + "-" * 60)
    status = "成功" if result.get("success") else "未完成"
    print(f"状态: {status}")
...
def save_to_jsonl(data: dict, filename: str):
    """Append one result record to a JSONL file."""
    try:
...
def should_use_code_mode(task: str) -> bool:
    """Detect explicit code-generation requests in interactive mode."""
    task_lower = task.lower()
    if task_lower.startswith("code:"):
        return True
    ...
    return any(marker in task_lower for marker in code_markers)
...
def interactive_mode():
    ...
    while True:
        try:
            ...
            generate_code = should_use_code_mode(task)
            if task.lower().startswith("code:"):
                task = task[5:].strip()

            if generate_code:
                print("\n代码模式：将生成包含 Python 代码示例的数据")
...
def batch_mode(input_file: str, output_file: str, max_workers: int = 2):
    ...
    try:
        with open(input_file, "r", encoding="utf-8") as file_handle:
            tasks = [line.strip() for line in file_handle if line.strip()]
...
def analyze_mode(input_file: str):
    """Analyze an existing JSONL dataset."""
    print(f"\n分析数据集: {input_file}")
    ...
    if stats.get("invalid_count", 0) > 0:
        print(f"  无效数据: {stats['invalid_count']} (已过滤)")
